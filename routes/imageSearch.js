...
async function fetchNaverImages(query) {
  if (!process.env.NAVER_CLIENT_ID || !process.env.NAVER_CLIENT_SECRET) return { needsApiKey: true, images: [] };
...
router.get('/', async (req, res) => {
  ...
  try {
    const result = await fetchNaverImages(query);
    if (result.needsApiKey) {
      return res.json({ needsApiKey: true, message: '네이버 검색 API 키가 없어서 이미지 검색을 못 해요.' });
    }
    res.json(result);
  } catch (err) {
...
module.exports.fetchNaverImages = fetchNaverImages;
