...
async function fetchGoogleImages(query) {
  if (!process.env.GOOGLE_API_KEY || !process.env.GOOGLE_CSE_ID) return { needsApiKey: true, images: [] };
  ...
  // 다음(Daum)과 같은 이유로 title을 query로 채우지 않는다 — 관련도 필터를 정직하게 유지한다.
...
router.get('/', async (req, res) => {
  ...
  try {
    const result = await fetchGoogleImages(query);
...
module.exports.fetchGoogleImages = fetchGoogleImages;
